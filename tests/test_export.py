...
class TestDateRangeToUtc:
    def test_utc_boundary_start_inclusive(self, admin_client, db_session):
        ...
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="BoundaryStart")
            entry = HarvestEntry(
    ...
    def test_utc_boundary_before_start_excluded(self, admin_client, db_session):
        ...
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="BeforeStart")
            entry = HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"),
    ...
    def test_utc_boundary_before_end_included(self, admin_client, db_session):
        ...
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="BeforeEnd")
            entry = HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"),
    ...
    def test_utc_boundary_at_end_excluded(self, admin_client, db_session):
        ...
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="AtEnd")
            entry = HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"),
                created_at=end_utc,
                worker_assignment_id=assignment.id,
...
class TestVigenteEntry:
    def test_active_entry_appears(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="Worker A")
            entry = HarvestEntry(
        ...
        assert ws.cell(row=row, column=11).value == "Vigente"
        assert ws.cell(row=row, column=13).value is None

        ws_res = wb["Resumen"]
        assert ws_res.cell(row=2, column=1).value == "Worker A"
        assert ws_res.cell(row=2, column=4).value == 1
        assert ws_res.cell(row=2, column=5).value == Decimal("45.500")
...
class TestAnuladoEntry:
    def test_voided_entry_shows_reason(self, admin_client, db_session):
        ...
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="Worker B")
            entry = HarvestEntry(
        ...
        for r in range(2, ws.max_row + 1):
            if ws.cell(row=r, column=1).value == entry_id:
                ...
                break
        assert row is not None
        assert ws.cell(row=row, column=11).value == "Anulado"
...
class TestInactiveWorker:
    def test_inactive_worker_included_in_export(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="Inactive W", active=False)
            entry = HarvestEntry(
...
class TestResumenTotals:
    def test_separate_vigente_anulado_totals(self, admin_client, db_session):
        ...
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="TotalWorker")
...
class TestDecimalWeightPrecision:
    def test_exact_decimal_sum(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="DecWorker")
            db_session.add(HarvestEntry(
        ...
        for r in range(2, ws_mov.max_row + 1):
            cell = ws_mov.cell(row=r, column=7)
        ...
        assert Decimal(str(ws_res.cell(row=total_row, column=5).value)).quantize(Decimal("0.001")) == Decimal("6.666")


class TestWeightCellFormat:
    def test_weight_cells_are_numeric(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="FmtWorker")
            db_session.add(HarvestEntry(
...
class TestDateAndTimeCellTypes:
    def test_date_and_time_are_excel_types_without_tzinfo(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="DTWorker")
            entry = HarvestEntry(
    ...
    def test_voided_at_is_datetime_without_tzinfo(self, admin_client, db_session):
        ...
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="VAWorker")
            entry = HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("5.000"),
                created_at=FIXED_NOW,
                voided=True, voided_at=voided_at,
        ...
        for r in range(2, ws.max_row + 1):
            if ws.cell(row=r, column=1).value == entry_id:
                ...
                break
        assert row is not None
        void_cell = ws.cell(row=row, column=12)
...
class TestFilterQ:
    def test_filter_by_name(self, admin_client, db_session):
        with db_session.begin_nested():
            w1, a1 = make_worker_with_assignment(db_session, name="XAlpha")
            w2, a2 = make_worker_with_assignment(db_session, name="XBeta")
            db_session.add(HarvestEntry(
                worker_id=w1.id, weight_kg=Decimal("1.000"), created_at=FIXED_NOW,
            ...
            db_session.add(HarvestEntry(
                worker_id=w2.id, weight_kg=Decimal("2.000"), created_at=FIXED_NOW,
    ...
    def test_filter_by_barcode(self, admin_client, db_session):
        with db_session.begin_nested():
            w1, a1 = make_worker_with_assignment(db_session, name="XOne")
            w2, a2 = make_worker_with_assignment(db_session, name="XTwo")
            db_session.add(HarvestEntry(
                worker_id=w1.id, weight_kg=Decimal("1.000"), created_at=FIXED_NOW,
                worker_assignment_id=a1.id,
                worker_slot_number_snapshot=w1.slot_number,
        ...
        resp = admin_client.get(
            f"/api/reports/harvest/export?start_date=2026-06-15&end_date=2026-06-15&q={w1.barcode}"
        )
        wb = load_workbook(io.BytesIO(resp.data))
        ws = wb["Movimientos"]
        names = [ws.cell(row=r, column=4).value for r in range(2, ws.max_row + 1)]
...
class TestFormulaProtectionInColumns:
    def test_formula_in_name_column_is_escaped(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="=CMD('calc.exe')")
            db_session.add(HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"), created_at=FIXED_NOW,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
                worker_barcode_snapshot=w.barcode,
                worker_name_snapshot=assignment.person_name,
    ...
    def test_formula_in_barcode_column_is_escaped(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="Safe")
            db_session.add(HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"), created_at=FIXED_NOW,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
...
class TestLegitimateApostrophe:
    def test_apostrophe_in_name_preserved(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="O'Brien")
            db_session.add(HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"), created_at=FIXED_NOW,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
...
class TestEntryOrdering:
    def test_ordered_by_created_at_then_id(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="OrderW")
            ts_later = datetime(2026, 6, 15, 10, 0, 0, tzinfo=timezone.utc)
            ts_same = datetime(2026, 6, 15, 9, 0, 0, tzinfo=timezone.utc)
            e_later = HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("3.000"), created_at=ts_later,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
                worker_barcode_snapshot=w.barcode,
                worker_name_snapshot=assignment.person_name,
            )
            e_first = HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"), created_at=ts_same,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
                worker_barcode_snapshot=w.barcode,
                worker_name_snapshot=assignment.person_name,
            )
            e_second = HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("2.000"), created_at=ts_same,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
                worker_barcode_snapshot=w.barcode,
                worker_name_snapshot=assignment.person_name,
...
class TestAutofilterAndFreeze:
    def test_freeze_always_a2(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="FreezeW")
            db_session.add(HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"), created_at=FIXED_NOW,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
    ...
    def test_resumen_autofilter_excludes_totals(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="AFWorker")
            db_session.add(HarvestEntry(
                worker_id=w.id, weight_kg=Decimal("1.000"), created_at=FIXED_NOW,
                worker_assignment_id=assignment.id,
                worker_slot_number_snapshot=w.slot_number,
...
class TestExistingReportBehavior:
    def test_json_report_unchanged(self, admin_client, db_session):
        with db_session.begin_nested():
            w, assignment = make_worker_with_assignment(db_session, name="JsonTest")
            db_session.add(HarvestEntry(
