...
def upgrade():
    ...
    op.add_column(
        'harvest_entries',
        sa.Column('rate_per_kg_snapshot', sa.Numeric(precision=8, scale=2), nullable=True),
    )
    op.add_column(
        'harvest_entries',
...
def downgrade():
    ...
    op.drop_constraint('ck_harvest_entries_product_snapshot_consistency', 'harvest_entries', type_='check')
    op.drop_constraint('ck_harvest_entries_amount_non_negative', 'harvest_entries', type_='check')
    op.drop_constraint('ck_harvest_entries_rate_snapshot_non_negative', 'harvest_entries', type_='check')
    op.drop_constraint('fk_harvest_entries_product_id', 'harvest_entries', type_='foreignkey')
